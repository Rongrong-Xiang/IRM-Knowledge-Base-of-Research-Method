...
entity_types = ["Question", "Data_Collection", "Data_Analysis"]
...
def train_fold(model, train_loader, val_loader, optimizer, device, max_epochs=30, early_stop=5):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i, pred_ids in enumerate(preds):
                ...
                # 保留所有源字段，只新增entities字段
                sample_result = dict(samples[len(results)])  # 浅拷贝源样本所有字段
                sample_result["entities"] = entities        # 增加预测实体
...
if __name__ == "__main__":
    all_samples = load_jsonl_data("/workspace/3_Innovation_method_202503/input_data/2_sample_manual_labeled_QM.jsonl")
    ...
    model = SciBERT_BiLSTM_CRF(model_dir, config, hidden_dim, len(label2id)).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=2e-5)
    train_fold(model, train_loader, train_loader, optimizer, device, max_epochs=10, early_stop=3)  # train/val都用全集即可
    torch.save(model.state_dict(), "/workspace/models/scibert_bilstm_crf_ner.pth")

    # 用全集模型做无标签数据预测
    keep_labels = ["Question", "Data_Collection", "Data_Analysis"]
    predict_unlabeled(
